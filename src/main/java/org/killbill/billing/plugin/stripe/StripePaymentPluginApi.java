...
public class StripePaymentPluginApi extends
        PluginPaymentPluginApi<StripeResponsesRecord, StripeResponses, StripePaymentMethodsRecord, StripePaymentMethods> {
    ...
    public List<PaymentTransactionInfoPlugin> getPaymentInfo(
        ...
        logger.info("AccountId:{} , PaymentId:{} , Properties:{}",
                kbAccountId,
                kbPaymentId,
                properties);

        logger.info("Getting transactions");
        final List<PaymentTransactionInfoPlugin> transactions = super.getPaymentInfo(
                kbAccountId, kbPaymentId,
                properties, context);

        if (transactions.isEmpty()) {
            logger.info("Transactions is empty:{}", transactions);
            // We don't know about this payment (maybe it was aborted in a control plugin)
            return transactions;
        }

        logger.info("Checking if a HPP payment needs to be canceled");
        ...
        logger.info("Expired payment policy: {}", expiredPaymentPolicy.toString());
        ...
        for (final PaymentTransactionInfoPlugin transaction : transactions) {
            ...
            if (transaction.getStatus() == PaymentPluginStatus.PENDING) {
                ...
                logger.info("Payment status: PENDING");

                final String paymentIntentId = PluginProperties.findPluginPropertyValue("id",
                        transaction.getProperties());

                try {
                    logger.info("Retrieve payment");
                    ...
                    // 3DS authorization failure - Fail payment according to property
                    else if (stripeConfigPropertiesConfigurationHandler.getConfigurable(context.getTenantId())
                            .isCancelOn3DSAuthorizationFailure()
                        ...
                        logger.info("Cancelling Stripe PaymentIntent after 3DS authorization failure {}",
                                intent.getId());
                    ...
                    logger.info("Last charge:{}", lastCharge.getAmount());

                    logger.info("Refresh payment (database)");
                    dao.updateResponse(
                    ...
                    logger.warn("Unable to refresh payment (database) : {}", e);
                ...
                logger.info("Payment status: UNDEFINED");
                ...
                try {
                    ...
                    if (result.getData().size() == 1) {
                        ...
                        logger.info("Canceling UNKNOWN Stripe transaction for kbTransactionId={}",
                                transaction.getKbTransactionPaymentId());
                        ...
                        final Map<String, Object> additionalMetadata = ImmutableMap.<String, Object>builder()
                                .put(PROPERTY_OVERRIDDEN_TRANSACTION_STATUS,
                                        PaymentPluginStatus.CANCELED.toString())
                                .put("message",
                                        "Payment didn't hapen - Cancelled by Janitor")
                                .build();
                        ...
                        try {
                            ...
                            logger.warn("Unable to update expired payment (database) : {}", e);
                            throw new PaymentPluginApiException("Unable to update expired payment", e);
                        }
    ...
    protected PaymentTransactionInfoPlugin buildPaymentTransactionInfoPlugin(final StripeResponsesRecord record) {
        logger.info("Build payment transaction info plugin");
    ...
    public PaymentMethodPlugin getPaymentMethodDetail(
        ...
        logger.info("Getting payment method detail");

        logger.info("AccountId:{} , PaymentMethodId:{} , Properties:{}",
                kbAccountId,
                kbPaymentMethodId,
                properties);
        ...
        try {
            ...
            logger.info("Unable to retrieve payment method for kbPaymentMethodId:{} Error:{}", kbPaymentMethodId, e);
    ...
    protected PaymentMethodPlugin buildPaymentMethodPlugin(final StripePaymentMethodsRecord record) {
        logger.info("Build payment method plugin");
    ...
    protected PaymentMethodInfoPlugin buildPaymentMethodInfoPlugin(final StripePaymentMethodsRecord record) {
        logger.info("Build payment method info plugin");
    ...
    public void addPaymentMethod(
        ...
        logger.info("AccountId:{} , PaymentMethodId:{} , Default:{} , Properties:{}",
                kbAccountId,
                kbPaymentMethodId,
                setDefault,
                properties);
        ...
        final Iterable<PluginProperty> allProperties = PluginProperties.merge(
        ...
        if (paymentMethodIdInStripe == null) {
            // Support also a source plugin property as it is a easier to pass it in cURLs
            // and recommended by Stripe
            paymentMethodIdInStripe = PluginProperties.findPluginPropertyValue("source", allProperties);

            if (paymentMethodIdInStripe != null) {
                objectType = "source";
            } else {
                // Support also a token plugin property as it is a bit easier to pass it in
                // cURLs (also sent by kbcmd in the body)
        ...
        logger.info("Payment method id in stripe:{}", paymentMethodIdInStripe);
        ...
        if (sessionId != null) {
            ...
            try {
                ...
                final StripeHppRequestsRecord hppRecord = dao.getHppRequest(
                        sessionId,
                ...
                if (hppRecord == null) {
                    ...
                    throw new PaymentPluginApiException("INTERNAL",
                            "Unable to add payment method: missing StripeHppRequestsRecord for sessionId " + sessionId);
                }

                final String setupIntentId = (String) StripeDao.fromAdditionalData(
                        hppRecord.getAdditionalData())
                        .get("setup_intent_id");
                ...
                if ("succeeded".equals(setupIntent.getStatus())) {
                    ...
                    if (existingCustomerId == null) {
                        ...
                        logger.info("Mapping kbAccountId {} to Stripe customer {}", kbAccountId,
                                setupIntent.getCustomer());

                        killbillAPI.getCustomFieldUserApi()
                                .addCustomFields(ImmutableList.of(new PluginCustomField(kbAccountId,
                                        ObjectType.ACCOUNT,
                                        "STRIPE_CUSTOMER_ID",
                                        setupIntent.getCustomer(),
                        ...
                        throw new PaymentPluginApiException("USER",
                                "Unable to add payment method : setupIntent customerId is " + setupIntent.getCustomer()
                                        + " but account already mapped to " + existingCustomerId);
                    }

                    // Used below to create the row in the plugin
                    // TODO This implicitly assumes the payment method type if "payment_method", is
                    // this always true?
                    paymentMethodIdInStripe = setupIntent.getPaymentMethod();
                } else {

                    throw new PaymentPluginApiException("EXTERNAL",
                            "Unable to add payment method: setupIntent status is: " + setupIntent.getStatus());
                }
            } catch (final SQLException e) {
        ...
        if (paymentMethodIdInStripe != null) {
            ...
            if ("payment_method".equals(objectType)) {
                ...
                try {
                    final PaymentMethod stripePaymentMethod = PaymentMethod.retrieve(
                            paymentMethodIdInStripe,
                            requestOptions);

                    additionalDataMap = StripePluginProperties.toAdditionalDataMap(stripePaymentMethod);

                    ImmutableMap<String, Object> params = ImmutableMap.of("payment_method",
                ...
                try {
                    ...
                    String customerId = createStripeCustomer(
                            kbAccountId,
                            existingCustomerId,
                            params,
                            requestOptions,
                    ...
                    stripeId = retrievePaymentMethod(
                            customerId,
                            existingCustomerId,
                ...
                try {
                    // The Stripe sourceId must be passed as the
                    // PaymentMethodPlugin#getExternalPaymentMethodId
                ...
                try {
                    // The Stripe bankAccountId must be passed as the
                    // PaymentMethodPlugin#getExternalPaymentMethodId
                    final PaymentSource paymentSource = Customer
                            .retrieve(existingCustomerId, expandSourcesParams, requestOptions)
                            .getSources()
        ...
        try {
            logger.info("Add payment method (database)");
    ...
    private String createStripeCustomer(
            final UUID kbAccountId,
        ...
        final String createStripeCustomerProperty = PluginProperties.findPluginPropertyValue("createStripeCustomer",
                allProperties);

        if (existingCustomerId == null
                && (createStripeCustomerProperty == null || Boolean.parseBoolean(createStripeCustomerProperty))) {
            ...
            // Stripe Customer exists OR creation is disabled: in those cases use the
            // default ID to charge
    ...
    private String retrievePaymentMethod(
        ...
        logger.info("Retrieve payment method");

        logger.info("Customer:{} , existingCustomer:{} , defaultStripeId:{}",
                customerId,
                existingCustomerId,
                defaultStripeId);
    ...
    protected String getPaymentMethodId(final StripePaymentMethodsRecord record) {
        logger.info("Get payment method id:{}", record.getKbPaymentMethodId());
    ...
    public void deletePaymentMethod(
        ...
        logger.info("AccountId:{} , PaymentMethodId:{}", kbAccountId, kbPaymentMethodId);
    ...
    public List<PaymentMethodInfoPlugin> getPaymentMethods(
        ...
        try {
            ...
            final List<StripePaymentMethodsRecord> existingStripePaymentMethodRecords = dao
                    .getPaymentMethods(kbAccountId, context.getTenantId());

            logger.info("Existing stripe payment method records (database):{}", existingStripePaymentMethodRecords);

            for (final StripePaymentMethodsRecord existingStripePaymentMethodRecord : existingStripePaymentMethodRecords) {
                existingPaymentMethodByStripeId.put(existingStripePaymentMethodRecord.getStripeId(),
                        existingStripePaymentMethodRecord);
            }
        } catch (final SQLException e) {
        ...
        // To retrieve all payment methods in Stripe, retrieve the Stripe customer id
        ...
        // Track the objects (the various Stripe APIs can return the same objects under
        // a different type)
        ...
        try {
            ...
            final Iterable<PaymentMethod> stripePaymentMethodsCard = PaymentMethod
                    .list(paymentMethodParams, requestOptions).autoPagingIterable(paymentMethodParams, requestOptions);

            syncPaymentMethods(kbAccountId,
                    stripePaymentMethodsCard,
                    existingPaymentMethodByStripeId,
                    stripeObjectsTreated,
                    context);

            paymentMethodParams.put("type", "sepa_debit");

            final Iterable<PaymentMethod> stripePaymentMethodsSepaDebit = PaymentMethod
                    .list(paymentMethodParams, requestOptions).autoPagingIterable(paymentMethodParams, requestOptions);

            syncPaymentMethods(kbAccountId,
                    stripePaymentMethodsSepaDebit,
                    existingPaymentMethodByStripeId,
                    stripeObjectsTreated, context);

            // Then go through the sources
            final PaymentSourceCollection psc = Customer.retrieve(stripeCustomerId, expandSourcesParams, requestOptions)
                    .getSources();

            if (psc != null) {

                final Iterable<? extends HasId> stripeSources = psc.autoPagingIterable(paymentMethodParams,
                        requestOptions);
        ...
        for (final StripePaymentMethodsRecord stripePaymentMethodsRecord : existingPaymentMethodByStripeId.values()) {
            ...
            logger.info("Deactivating local Stripe payment method {} - not found in Stripe",
                    stripePaymentMethodsRecord.getStripeId());

            super.deletePaymentMethod(
                    kbAccountId, UUID.fromString(stripePaymentMethodsRecord.getKbPaymentMethodId()),
                    properties,
                    context);
        }
    ...
    private void syncPaymentMethods(
        ...
        logger.info("Sync payment methods");

        logger.info("AccountId:{} , StripeObjects:{} , StripeObjectsTreated:{} , StripeObjectsTreated:{}",
                kbAccountId,
                stripeObjects,
                existingPaymentMethodByStripeId,
                stripeObjectsTreated);
        ...
        for (final HasId stripeObject : stripeObjects) {
            ...
            final StripePaymentMethodsRecord existingPaymentMethodRecord = existingPaymentMethodByStripeId
                    .remove(stripeObject.getId());
    ...
    public PaymentTransactionInfoPlugin authorizePayment(
        ...
        final boolean isHPPCompletion = stripeResponsesRecord != null && Boolean.valueOf(MoreObjects.firstNonNull(
                StripeDao.fromAdditionalData(stripeResponsesRecord.getAdditionalData()).get(PROPERTY_FROM_HPP), false)
                .toString());
        ...
        if (!isHPPCompletion) {
            ...
            // (the API can be called for instance after the user is redirected back from
            // the HPP)
        ...
        return executeFollowUpTransaction(TransactionType.REFUND,
                new TransactionExecutor<PaymentIntent>() {
                    ...
                    public PaymentIntent execute(final Account account,
                        ...
                        // The PaymentIntent API doesn't have a refund API - refund the charge created
